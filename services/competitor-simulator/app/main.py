...
logging.basicConfig(
    ...
    format="%(asctime)s %(levelname)s [competitor-simulator] %(message)s",
)
...
class CompetitorPriceSimulator:
    ...
    async def run(self) -> None:
        ...
        logger.info(
            "Competitor simulator started. Interval range: %ss-%ss. simulation_speed=%sx mix=50%% neutral 30%% drop 20%% increase",
    ...
    async def _simulate_one_change(self) -> None:
        ...
        async with AsyncSessionLocal() as session:
            ...
            logger.info(
                "Published competitor price change: product_id=%s product=%s competitor=%s old=%s new=%s change=%s%% scenario=%s stock=%s",
                product.id,
                product.name,
                competitor_name,
                old_price,
